...
ValineFactory.prototype.bind = function (option) {
    ...
    let addSmilies = () => {
        ...
        Object.keys(smiliesData).forEach((y, i) => {
            ...
            smiliesData[y].split('|').forEach(e => {
                _ul.innerHTML += `
                <li class="smilies-item" title="${e}" data-input="${(y == sl ? '@' : '#') +
                    `(${e})`}">
                    <img class="biaoqing ${y == sl ? 'newpaopao' : 'alu'}" title="${e}" src="${cdnprefix}${
                    y == sl ? 'newpaopao' : 'alu'
                    }/${e}.png">
                </li>
                `
    ...
    let submitEvt = (e) => {
        ...
        while ((matched = defaultComment.comment.match(aReg))) {
            defaultComment.comment = defaultComment.comment.replace(
                matched[0],
                `<img src="${cdnprefix}alu/${matched[1]}.png" class="biaoqing alu" height=33 width=33 no-zoom />`
            )
        }
